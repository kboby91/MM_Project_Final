...
int main()
{
	...
	vector<Rect> playersRectTrace;
	vector<Mat> playersImgTrace;
	...
	{		
		...
		for (size_t i = 0; i < playersRectDetected.size(); i++) {
			...
			Mat SubImage = imgOriginal(rect);
			Mat BackImage = TraceImage(rect);

			//SubImage.copyTo(BackImage, SubImage);
			cv::addWeighted(BackImage, 0.5, SubImage, 0.5, 0, BackImage);
		}

		cv::addWeighted(imgOriginal, 0.8, TraceImage, 1, 0, imgOriginal);
		imshow("Players detection with tracking_1", imgOriginal);
		imshow("backImage", TraceImage);
#endif
		...
		for (size_t i = 0; i < playersRectDetected.size(); i++)
		{
			...
			Rect rect(playersRectDetected[i].x, playersRectDetected[i].y, width, height);
			Rect rectT(playersRectDetected[i].x + width/2 - 2, playersRectDetected[i].y + height/2 - 2, 5, 5);
			
			if (result == 1) {
				rectangle(imgOriginal, rect, Scalar(255, 0, 0), 2, 8, 0);
				rectangle(TraceImage, rectT, Scalar(255, 0, 0), 2, 8, 0);
			}
			else if (result == 2) {
				rectangle(imgOriginal, rect, Scalar(0, 255, 0), 2, 8, 0);
				rectangle(TraceImage, rectT, Scalar(0, 255, 0), 2, 8, 0);
			}
			else {
				rectangle(imgOriginal, rect, Scalar(0, 0, 255), 2, 8, 0);
				rectangle(TraceImage, rectT, Scalar(0, 0, 255), 2, 8, 0);
			}			
		}
		
		//imshow("TraceImage", TraceImage);
		cv::addWeighted(TraceImage, 1, temp, 0.8, 0, temp);
		imshow("TraceImage", temp);
